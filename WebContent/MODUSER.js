...
var mm = String(currentDate.getMonth() + 1).padStart(2, '0'); // January is 0!
var yyyy = currentDate.getFullYear();
var hh = String(currentDate.getHours()).padStart(2, '0'); // Get hours and pad to 2 digits
var mi = String(currentDate.getMinutes()).padStart(2, '0'); // Get minutes and pad to 2 digits
var ss = String(currentDate.getSeconds()).padStart(2, '0'); // Get seconds and pad to 2 digits

currentDate = yyyy + '-' + mm + '-' + dd + ' ' + hh + ':' + mi + ':' + ss;
console.log(currentDate); // in 24-hour format
*/
...
/*  await swal("Wait", "Please wait while getting customer details", "success", {
 ...
 fetch(origin+"/"+parent.getProjectName()+"/Screen?function_id=MODUSER&method=GET&cust_id="+custId)  
.then(function(response) {  
 ...
 .catch(function(error) {                        // catch
  ...
  fetch(origin+"/"+parent.getProjectName()+"/Screen?function_id=MODUSER&method=GET&cust_id="+custId).then(data=> data.json())
 .then((data) => {
 ...
 if(key === "cust_Once_Auth" && value === "Y")
   ...
   /* else if(cust_mak !== userLogIndocument.getElementById("cust_Approver_statement").value === "U" && document.getElementById("cust_Once_Auth").value ==="N" ){
	 document.getElementById("ModifyId").disabled = true;
	   document.getElementById("ModifyId").style.display="none";  */
	   /* document.getElementById("CloseId").disabled = true;
	   document.getElementById("CloseId").style.display=""; */
...
function CustPost(method){
	...
	let cust_pass = document.getElementById("cust_password").value;
	...
	let cust_manager = document.getElementById("cust_manager").value;
	let cust_ladp = document.getElementById("cust_ldap").value;
		...
		
	{
		if(method === "POST")
		{
		   url_send_data=origin+"/"+parent.getProjectName()+"/Screen?function_id=MODUSER&method=POST&cust_id="+custId+"&"+"cust_name="+cust_name+"&"+
	  ...
	  if(cust_mak !== userLogIn && document.getElementById("cust_Approver_statement").value === "Unauthorize"  ){
			...
			if (document.getElementById("cust_Once_Auth").value="Y" && document.getElementById("cust_Approver_statement").value === "Unauthorize")	   {
				 ...
				 modifybtn();
		 		 ...
		 		 url_send_data=origin+"/"+parent.getProjectName()+"/Screen?function_id=MODUSER&method=UPDATE&cust_id="+custId+"&"+"cust_name="+cust_name+"&"+
				 "cust_password="+cust_pass+"&"+"cust_dept="+cust_dept+"&"+"cust_email="+cust_email+"&"+"cust_manager="+cust_manager+"&"+"cust_ldap="+cust_ladp+"&"+"cust_phone="+cust_phone+"&"+"cust_maker="+cust_maker+"&"+
				 "cust_maker_dt_stamp="+cust_maker_dt_stamp+"&"+"cust_Approver="+cust_Approver+"&"+"cust_Aprovertime="+cust_Aprovertime+
				 "&"+"cust_Status="+cust_Status+"&"+"cust_Approver_statement="+cust_Approver_statement+"&"+"cust_Once_Auth="+cust_Once_Auth+"&"+"countRow="+countRow;  
...
function getFrontEndData(method){
	...
	var userLogIn=parent.document.getElementById("user_id").innerText.trim();
	console.log("userLogIn:",userLogIn);
	let custId = document.getElementById("cust_id").value.trim();
	console.log(custId);
	let cust_name = document.getElementById("cust_name").value.trim();
	console.log(cust_name);
	let cust_pass = document.getElementById("cust_password").value.trim();
	console.log(cust_pass);
	let cust_dept = document.getElementById("cust_dept").value.trim();
	console.log(cust_dept);
	let cust_email = document.getElementById("cust_email").value.trim();
	console.log(cust_email);
	/* let cust_maker = document.getElementById("cust_maker").value; */
	let cust_manager = document.getElementById("cust_manager").value.trim();
	console.log(cust_manager);
	let cust_ladp = document.getElementById("cust_ldap").value.trim();
	console.log(cust_ladp);
	let cust_phone = document.getElementById("cust_phone").value.trim();
	console.log(cust_phone);
	let cust_maker = parent.document.getElementById("user_id").innerText.trim();
	console.log(cust_maker);
	let cust_maker_dt_stamp = document.getElementById("cust_maker_dt_stamp").value.trim();
	console.log(cust_maker_dt_stamp);
	let cust_Approver = document.getElementById("cust_Approver").value.trim();
	console.log(cust_Approver);
	let cust_Aprovertime = document.getElementById("cust_Aprovertime").value.trim();
	console.log(cust_Aprovertime);
	let cust_Status = document.getElementById("cust_Status").value.trim();
	console.log(cust_Status);
	let cust_Approver_statement = document.getElementById("cust_Approver_statement").value.trim();
	console.log(cust_Approver_statement);
	let cust_Once_Auth=document.getElementById("cust_Once_Auth").value.trim();
	console.log(cust_Once_Auth);
	let countRow=document.getElementById("countRow").value.trim();
...
function deleteData(){
	...
	fetch(origin+"/"+parent.getProjectName()+"/Screen?function_id=MODUSER&method=DELETE&cust_id="+custId);
	...
	if( cust_makerE === parent_cust_makerElem)
		{
		   ...
		   if(cust_makerE === parent_cust_makerElem && document.getElementById("cust_Approver_statement").value === "Unauthorize")
				 document.getElementById("DeleteId").style.display=""; 
			if(cust_makerE === parent_cust_makerElem  && document.getElementById("cust_Once_Auth").value === "Y")
				   document.getElementById("DeleteId").style.display="none"; 
		}
...
function approveData(){
	...
	if(document.getElementById("cust_maker").value !== parent.document.getElementById("user_id").innerText.trim() && document.getElementById("cust_Approver_statement").value === "Unauthorize" && document.getElementById("cust_Once_Auth").value === "N"){
		 ...
		 fetch(origin+"/"+parent.getProjectName()+"/Screen?function_id=MODUSER&method=APPROVE&cust_id="+custId+"&"+"cust_name="+cust_name+"&"+
		 "cust_password="+cust_pass+"&"+"cust_dept="+cust_dept+"&"+"cust_email="+cust_email+"&"+"cust_manager="+cust_manager+"&"+"cust_ldap="+cust_ladp+"&"+"cust_phone="+cust_phone+"&"+"cust_maker="+cust_mak+"&"+
		 "cust_maker_dt_stamp="+cust_maker_dt_stamp+"&"+"cust_Approver="+ cust_Approver+"&"+"cust_Aprovertime="+cust_Aprovertime+
...
//	let cust_Aprovertime = document.getElementById("cust_Aprovertime").value;
	...
	console.log("reopen====");
	 	...
	 	fetch(origin+"/"+parent.getProjectName()+"/Screen?function_id=MODUSER&method=REOPEN&cust_id="+custId+"&"+"cust_name="+cust_name+"&"+
		 "cust_password="+cust_pass+"&"+"cust_dept="+cust_dept+"&"+"cust_email="+cust_email+"&"+"cust_manager="+cust_manager+"&"+"cust_ldap="+cust_ladp+"&"+"cust_phone="+cust_phone+"&"+"cust_maker="+cust_mak+"&"+
		 "cust_maker_dt_stamp="+cust_maker_dt_stamp+"&"+"cust_Approver="+cust_Approver+"&"+"cust_Aprovertime="+cust_Aprovertime+
...
function CloseAcc(){
	...
	alert("Cannot Closed");
		...
		fetch(origin+"/"+parent.getProjectName()+"/Screen?function_id=MODUSER&method=CLOSE&cust_id="+custId+"&"+"cust_name="+cust_name+"&"+
				 "cust_password="+cust_pass+"&"+"cust_dept="+cust_dept+"&"+"cust_email="+cust_email+"&"+"cust_manager="+cust_manager+"&"+"cust_ldap="+cust_ladp+"&"+"cust_phone="+cust_phone+"&"+"cust_maker="+cust_mak+"&"+
				 "cust_maker_dt_stamp="+cust_maker_dt_stamp+"&"+"cust_Approver="+cust_Approver+"&"+"cust_Aprovertime="+cust_Aprovertime+
				 "&"+"cust_Status="+cust_StatusE+"&"+"cust_Approver_statement="+cust_Approver_statement+"&"+"cust_Once_Auth="+cust_Once_Auth+"&"+"countRow="+countRow);
 ...
 */
 ...
 function enbFilds(){
	/* var inputs = document.querySelectorAll('.disable-input');
	 ...
	 /*  document.getElementById("buttonSave").enabled = true;
		...
		document.getElementById("cust_id").style.enabled=true;
		document.getElementById('cust_id').disabled = true;
...
/* function checkDuplicate() {
  ...
  const id = idField.value.trim(); // get the ID entered by the user

  if (ids.has(id)) { // check if the ID already exists in the Set
    alert("Duplicate ID! Please enter a unique ID."); // show an alert message if a duplicate is found
    idField.value = ""; // clear the input field
    return;
  }

  ids.add(id); // add the ID to the Set if it's unique
  idField.value = ""; // clear the input field
} */
...
/*    window.addEventListener("message", function(message) {
console.log("Message received from the parent: " + message.data.role_id); // Message received from parent
 recivedArray[0]= message.data[0].trim(); 
console.log("Message received from the parent: " + message.data.role_desc);

 recivedArray[1] = message.data[1].trim(); 
console.log(message.data[0]);
console.log(message.data[1]);
document.getElementById("inputtd0").value = message.data[0].trim();
document.getElementById("inputtd10").value = message.data[1].trim();
closeFrame1();
...
/* const iframeDouc=doucment.getElementById('iframeChild');
console.log("Iframe Data:::",iframeDouc); */
...
*/			  
	...
	/* 
	  ...
	  myWindow.document.write('<iframe  name="SearchFrameId" id="iframeChild" height="500px"  allowTransparency="true"  frameborder="0" scrolling="yes" style="width:100%;" src="'+ifrem.src+'" type= "text/javascript"></iframe>');   
...
/* var rowIds = [];
//Inside the JavaScript code that creates the table rows
var table = document.getElementById('myTable');
var rowCount =	document.getElementById('countRow').value = table.tBodies[0].rows.length;
for (var i = 0; i < rowCount; i++) {
  var row = document.createElement('tr');
  var rowId = 'inputid' + i; // Generate the row ID dynamically

  row.setAttribute('id', rowId);
  rowIds.push(rowId); // Store the row ID in the array
	...
	var cell3 = row.insertCell(2);
	 var element3 = document.createElement("img");
	   ...
	   element3.id = "searcher" + c;
		...
		/*  for(var index=0;index<document.getElementById('countRow').value;index++){
			  var inputTD="inputtd";
			  var inputTD1="inputtd1";
			  
			  console.log("inputTD::"+inputTD);
			  console.log("inputTD1::"+inputTD1); 
		  /* inputdesc="inputdesc"+c;
		  inputid="inputid"+c; */
		  /* var frameinVar="frameinWin1('LOV_MODUSER_ROLE','MODUSER','List of Values ROLES');insertData('"+inputdesc+"','"+inputid+"')"; */
 	...
 	/*
	 var copycel = document.getElementById('sBox0').innerHTML;
